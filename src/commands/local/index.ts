...
/** `deviations list|get|resolve|discover`. */
const cmdDeviations = async (ctx: Context, args: string[]): Promise<void> => {
  ...
  switch (verb) {
    ...
    default:
      throw new CliError(
        verb === undefined ? "missing deviations verb (list|get|resolve|discover)" : `unknown deviations verb "${verb}"`,
      );
  }
};
 ...
 * `discovery run` alias — identical flags (--sample, --flow, wait flags),
 * the same `/api/discovery/run` enqueue, no behavior difference.
 */
...
/** `discovery run` — alias of the canonical `deviations discover`. */
...
/** `experiments list|get <id>` — read-only mirror of the loopback `/api/experiments` endpoints. */
const cmdExperiments = async (ctx: Context, args: string[]): Promise<void> => {
  ...
  switch (verb) {
    ...
    default:
      throw new CliError(
        verb === undefined ? "missing experiments verb (list|get)" : `unknown experiments verb "${verb}"`,
      );
  }
};
